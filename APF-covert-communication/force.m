...
if der==1
    ...
    elseif changflag==2
        % 部分场位于遮蔽情况下
          if abs(rw0-sqrt(snr2/snr1-H^2))<boud_ds
                F=-1*snr2/log(2)/(r0^2+H^2)/(r0^2+H^2+snr2)*(r(1))+...
                  snr1/log(2)/(r0^2+H^2+(rw0^2+H^2)*snr1)*((rw0^2+H^2)/(r0^2+H^2)*(-r(1))+rw(1));
          else
...
elseif der==2
    ...
    elseif changflag==2
        % 部分场位于遮蔽情况下
            if abs(rw0-sqrt(snr2/snr1-H^2))<boud_ds
                F=-1*snr2/log(2)/(r0^2+H^2)/(r0^2+H^2+snr2)*(r(2))+...
                    snr1/log(2)/(r0^2+H^2+(rw0^2+H^2)*snr1)*((rw0^2+H^2)/(r0^2+H^2)*(-r(2))+rw(2)); 
            else
                if snr1*(rw0^2+H^2)>snr2
                    F=-1*snr2/log(2)/(r0^2+H^2)/(r0^2+H^2+snr2)*(r(2));
                else
