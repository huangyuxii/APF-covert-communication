    ...
    for y=-50:10:100
        for z=0:10:50
            ...
            if   snr1*sigma^2/beta*(rw0^2+H^2)>Pmax
                ...
                FX(xi,yi,zi)=-2/log(2)/(1+snr1/r0^2)*snr1/r0^3*r(1)+ 1/log(2)/(1+snr1/r0^2)*snr1/r0^2*2*rw0*rw(1);
                FX(xi,yi,zi)=-2/log(2)/(1+snr1/r0^2)*snr1/r0^3*r(2)+ 1/log(2)/(1+snr1/r0^2)*snr1/r0^2*2*rw0*rw(2);
                FX(xi,yi,zi)=-2/log(2)/(1+snr1/r0^2)*snr1/r0^3*r(3)+ 1/log(2)/(1+snr1/r0^2)*snr1/r0^2*2*rw0*rw(3);
            end
            zi=zi+1;
        end
        zi=1;
...
quiver3(X,Y,Z,FX,FY,FZ);hold on;
xlabel('x[m]');
ylabel('y[m]');
...
quiver3(X*5+10,Y*5,Z*5,U,V,W,'r'),hold on;

[X,Y,Z] = sphere(20);
[U,V,W] = surfnorm(X,Y,Z);
quiver3(-X*25,-Y*25,-Z*25,U,V,W,'b'),hold on;
zlim([0 5]);
axis equal
